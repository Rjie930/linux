...
static int __init led_init(void)
{
	...
	err=request_irq(irq,btn_interrupt,IRQF_TRIGGER_FALLING,"K2",NULL);
	
	alloc_chrdev_region(&dev,0,1,"key2_dev");
	p_cdev = cdev_alloc();
	cdev_init(p_cdev,&fops);
	cdev_add(p_cdev,dev,1);
	led_class=class_create(THIS_MODULE,"key2_dev");
	device_create(led_class,NULL,dev,NULL,"key2_dev");
	return 0;
}
